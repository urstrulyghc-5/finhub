...
const DOMAINS = [
  ...
  { id: 'corporate', icon: 'corporate', name: 'Corporate Finance',
    ...
    categories: [
      ...
      { id: 'financing-decision', name: 'Financing Decisions', subcategories: ['Cost of Capital', 'Capital Structure', 'Leverage', 'Debt vs Equity'] },
      ...
      { id: 'valuation', name: 'Corporate Valuation', subcategories: ['Discounted Cash Flow', 'Terminal Value', 'Enterprise Value', 'Relative Valuation'] },
...
/* ---- the voyage ---- */
.voy{margin-top:30px;border:1px solid var(--line);border-radius:18px;overflow:hidden;
  background:var(--surface);box-shadow:var(--shadow)}
.voy-svg{display:block;width:100%;height:auto;aspect-ratio:100/74;max-height:340px}
.voy-far{fill:var(--muted);opacity:.16}
.voy-near{fill:var(--teal);opacity:.20}
.voy-route{fill:none;stroke:var(--line);stroke-width:.5;stroke-dasharray:2 2}
.voy-route-done{fill:none;stroke:var(--amber);stroke-width:.7;stroke-linecap:round;
  transition:stroke-dashoffset .25s linear}
.voy-port{fill:var(--line);transition:fill .5s ease,r .4s ease}
.voy-port.on{fill:var(--amber);r:1.7}
.voy-swell path{fill:none;stroke:var(--teal);stroke-width:.5;opacity:.45}
.voy-swell.move{animation:swell 7s ease-in-out infinite alternate}
@keyframes swell{from{transform:translateX(0)}to{transform:translateX(-4px)}}
.voy-ship{transition:transform .25s linear}
.voy-hull{fill:var(--text)}
.voy-mast{stroke:var(--text);stroke-width:.45}
.voy-sail{fill:var(--surface);stroke:var(--text);stroke-width:.35}
.voy-sail.two{fill:var(--amber-soft)}
.voy-lamp{fill:var(--amber)}
.voy-label{padding:14px 20px;font-size:13.5px;color:var(--muted);
  border-top:1px solid var(--line);line-height:1.55}
@media(prefers-reduced-motion:reduce){.voy-swell.move{animation:none}.voy-ship{transition:none}}
...
function Voyage({ progress = 0, label }) {
  const reduced = useReducedMotion();
  const p = Math.max(0, Math.min(1, progress));
  const x = 6 + p * 84;
  const y = 52 + Math.sin(p * Math.PI * 2.2) * 5;
  const tilt = Math.cos(p * Math.PI * 2.2) * 6;

  return (
    <div className="voy" aria-hidden="true">
      <svg viewBox="0 0 100 74" className="voy-svg" preserveAspectRatio="xMidYMid meet">
        <defs>
          ...
          <linearGradient id="voy-sea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="var(--teal)" stopOpacity=".20" />
            <stop offset="100%" stopColor="var(--teal)" stopOpacity=".04" />
          </linearGradient>
        </defs>

        <rect x="0" y="0" width="100" height="74" fill="url(#voy-sky)" />

        {/* far coast, near coast: two layers for depth */}
        <path className="voy-far" d="M0 44 L12 40 L20 43 L30 38 L40 42 L52 37 L64 41 L74 38 L86 42 L100 39 L100 48 L0 48 Z" />
        <path className="voy-near" d="M0 50 L14 47 L26 50 L38 46 L50 49 L62 45 L76 49 L88 46 L100 49 L100 56 L0 56 Z" />

        {/* the route, drawn as the journey advances */}
        <path id="voy-route" className="voy-route"
          d="M6 52 Q26 44 44 54 T78 50 T94 53" />
        <path className="voy-route-done" d="M6 52 Q26 44 44 54 T78 50 T94 53"
          style={{ strokeDasharray: 120, strokeDashoffset: 120 - p * 120 }} />

        {/* ports along the way */}
        {[[6,52],[30,49],[54,52],[78,50],[94,53]].map(([px,py],i)=>(
          <circle key={i} cx={px} cy={py} r="1.3"
            className={"voy-port" + (p * 4 >= i ? " on" : "")} />
        ))}

        {/* swell */}
        <g className={"voy-swell" + (reduced ? "" : " move")}>
          <path d="M0 58 Q10 56 20 58 T40 58 T60 58 T80 58 T100 58" />
          <path d="M0 63 Q12 61 24 63 T48 63 T72 63 T96 63" opacity=".6" />
          <path d="M0 68 Q14 66 28 68 T56 68 T84 68 T112 68" opacity=".35" />
        </g>

        {/* the ship */}
        <g className="voy-ship" transform={`translate(${x} ${y}) rotate(${tilt})`}>
          <path className="voy-hull" d="M-5 1 L5 1 L3.4 4 L-3.4 4 Z" />
          <line className="voy-mast" x1="0" y1="1" x2="0" y2="-7.5" />
          <path className="voy-sail" d="M0.5 -7 L5 -1.5 L0.5 -1.5 Z" />
          <path className="voy-sail two" d="M-0.5 -6 L-4 -1.5 L-0.5 -1.5 Z" />
          <circle className="voy-lamp" cx="0" cy="-8.2" r="0.8" />
        </g>
      </svg>
      ...
      <div className="wrap">
        ...
        <Reveal delay={220}>
          <Voyage progress={progress}
            label="Value travelled by sea for four thousand years. Scroll, and the voyage moves with you." />
        </Reveal>
      </div>
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <Reveal delay={170}>
          <Voyage progress={progress}
            label="From the banyan tree to the matching engine. The voyage advances as you read." />
        </Reveal>
      </div>
