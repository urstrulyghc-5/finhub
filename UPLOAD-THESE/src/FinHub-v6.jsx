...
function Voyage({ progress = 0, label, ports = [] }) {
  const reduced = useReducedMotion();
  const p = Math.max(0, Math.min(1, progress));
  const x = 8 + p * 82;
  const bob = Math.sin(p * Math.PI * 5) * 1.6;
  const y = 56 + bob;
  const tilt = Math.cos(p * Math.PI * 5) * 4;
  const reached = Math.round(p * (ports.length - 1));

  return (
    <div className="voy" aria-hidden="true">
      <svg viewBox="0 0 100 60" className="voy-svg" preserveAspectRatio="none">
        <defs>
          <linearGradient id="voy-sky" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="var(--amber-soft)" />
            <stop offset="55%" stopColor="var(--surface)" />
            <stop offset="100%" stopColor="var(--teal-soft)" />
          </linearGradient>
          <radialGradient id="voy-sun" cx="0.5" cy="0.5">
            <stop offset="0%" stopColor="var(--amber)" stopOpacity=".45" />
            <stop offset="100%" stopColor="var(--amber)" stopOpacity="0" />
          </radialGradient>
        </defs>

        <rect width="100" height="60" fill="url(#voy-sky)" />
        <circle cx="82" cy="16" r="14" fill="url(#voy-sun)" />
        <circle cx="82" cy="16" r="3.2" className="voy-sun-core" />

        {/* three layers of coast, each drifting at its own speed for depth */}
        <g className={"voy-layer far" + (reduced ? "" : " drift")}>
          <path d="M0 34 L10 30 L18 33 L28 28 L38 32 L50 27 L62 31 L72 28 L84 32 L96 29 L110 33 L110 40 L0 40 Z" />
          <path d="M110 34 L120 30 L128 33 L138 28 L148 32 L160 27 L172 31 L182 28 L194 32 L206 29 L220 33 L220 40 L110 40 Z" />
        </g>
        <g className={"voy-layer mid" + (reduced ? "" : " drift")}>
          <path d="M0 39 L14 36 L26 39 L38 35 L52 38 L66 34 L80 38 L94 35 L110 38 L110 44 L0 44 Z" />
          <path d="M110 39 L124 36 L136 39 L148 35 L162 38 L176 34 L190 38 L204 35 L220 38 L220 44 L110 44 Z" />
        </g>

        {/* the route across the whole width */}
        <path className="voy-route" d="M8 56 Q30 50 50 57 T90 55" />
        <path className="voy-route-done" d="M8 56 Q30 50 50 57 T90 55"
          style={{ strokeDasharray: 100, strokeDashoffset: 100 - p * 100 }} />

        {ports.map((label, i) => {
          const px = 8 + (i / Math.max(1, ports.length - 1)) * 82;
          const on = i <= reached;
          return (
            <g key={i} className={"voy-port-g" + (on ? " on" : "")}>
              <line x1={px} y1="44" x2={px} y2="53" className="voy-post" />
              <circle cx={px} cy="53.5" r="1.5" className="voy-port" />
            </g>
          );
        })}

        {/* swell, always in motion */}
        <g className={"voy-swell" + (reduced ? "" : " move")}>
          <path d="M-20 50 Q0 48 20 50 T60 50 T100 50 T140 50" />
          <path d="M-20 54 Q5 52 30 54 T80 54 T130 54" opacity=".55" />
        </g>
        <g className={"voy-swell two" + (reduced ? "" : " move")}>
          <path d="M-20 58 Q10 56 40 58 T100 58 T160 58" opacity=".4" />
        </g>

        <g className="voy-ship" transform={`translate(${x} ${y}) rotate(${tilt}) scale(1.35)`}>
          <path className="voy-wake" d="M-6 3.6 Q-11 4.2 -15 3.4" />
          <path className="voy-hull" d="M-5 1 L5 1 L3.2 4 L-3.2 4 Z" />
          <line className="voy-mast" x1="0" y1="1" x2="0" y2="-8" />
          <path className="voy-sail" d="M0.6 -7.6 L5.4 -1.4 L0.6 -1.4 Z" />
          <path className="voy-sail two" d="M-0.6 -6.4 L-4.4 -1.4 L-0.6 -1.4 Z" />
          <path className="voy-flag" d="M0 -8 L2.6 -7.2 L0 -6.4 Z" />
        </g>
      </svg>

      <div className="voy-foot">
        {ports.length > 0 && (
          <div className="voy-ports">
            {ports.map((n, i) => (
              <span key={n} className={"voy-pname" + (i <= reached ? " on" : "")}>{n}</span>
            ))}
          </div>
        )}
        {label && <p className="voy-label">{label}</p>}
      </div>
    </div>
  );
}
      ...
      <div className="wrap">
        ...
        <Reveal delay={220}>
          <Voyage progress={progress}
            ports={["Coinage", "Statecraft", "Hundi & bills", "Indian Ocean", "Central banks", "Modern era"]}
            label="Value travelled by sea for four thousand years. The hundi crossed the Indian Ocean, bills of exchange crossed the Mediterranean, and joint stock companies existed to fund voyages. Scroll, and the journey advances with you." />
        </Reveal>
      </div>
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <Reveal delay={170}>
          <Voyage progress={progress}
            ports={["Before exchanges", "Joint stock", "Certificates", "The floor", "Screens", "Demat", "The pocket"]}
            label="From merchants under a banyan tree to a matching engine that never sleeps. The voyage advances as you read." />
        </Reveal>
      </div>
