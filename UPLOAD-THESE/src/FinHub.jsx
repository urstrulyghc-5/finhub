...
const NAV = [
  ["Start here", "#/types"], ["Universe", "#/universe"], ["Origins", "#/origins"], ["History", "#/history"], ["Data", "#/data"], ["Tax", "#/tax"], ["Intelligence", "#/intelligence"], ["Payments", "#/payments"], ["Simulator", "#/floor"], ["Telemetry", "#/telemetry"], ["Concepts", "#/concepts"], ["Cases", "#/cases"],
...
/* ---- Payment simulator: why one payment lands in a second and another
       takes three days. Every step shown, with the delay named. ---- */
const PAY_ROUTES = {
  upi: {
    name: "UPI, domestic", total: "Under 2 seconds", tone: "hi",
    note: "A single domestic system, one currency, one regulator, one clearing house. Every participant is already connected to the same rails, so nothing has to be translated or re-verified.",
    steps: [
      { s: "Initiation", ms: 120, t: "You approve the payment in an app. The instruction is signed and sent." },
      { s: "Authentication", ms: 260, t: "Your PIN authorises the debit. The bank confirms the account and balance." },
      { s: "Routing", ms: 180, t: "The central switch identifies the beneficiary bank from the identifier." },
      { s: "Debit and credit", ms: 300, t: "Payer debited, payee credited. Both banks confirm." },
      { s: "Settlement", ms: 480, t: "Banks settle between themselves in cycles. The customer already sees the money." },
    ],
  },
  card: {
    name: "Card payment", total: "Seconds to authorise, days to settle", tone: "",
    note: "Authorisation and settlement are separate processes. The merchant sees approval instantly, and the money arrives later, which is why refunds take days.",
    steps: [
      { s: "Authorisation", ms: 900, t: "The terminal asks the issuer whether funds exist. A hold is placed, not a transfer." },
      { s: "Capture", ms: 1200, t: "At the end of the day the merchant submits the day's approved transactions." },
      { s: "Clearing", ms: 1800, t: "The network calculates what each bank owes each other bank." },
      { s: "Settlement", ms: 2400, t: "Funds move between banks, typically one to three days after the purchase." },
      { s: "Merchant credit", ms: 1600, t: "The acquirer credits the merchant, less fees." },
    ],
  },
  cross: {
    name: "Cross-border transfer", total: "One to five working days", tone: "bad",
    note: "No single system spans both countries. The payment is handed between institutions, each applying its own checks, in different time zones, with a currency conversion in the middle.",
    steps: [
      { s: "Initiation", ms: 700, t: "You instruct your bank. It verifies your identity and the purpose of the payment." },
      { s: "Compliance screening", ms: 2200, t: "Sanctions and anti-money-laundering checks. This step alone can hold a payment for hours or days." },
      { s: "Correspondent routing", ms: 2600, t: "Your bank may have no relationship with the destination bank, so the payment passes through one or more intermediaries, each taking a fee." },
      { s: "Currency conversion", ms: 1800, t: "Converted at a rate set by the converting institution, usually with a spread above the market rate." },
      { s: "Time zone wait", ms: 2800, t: "Each institution processes in its own working hours. A payment sent on Friday evening waits for Monday." },
      { s: "Beneficiary credit", ms: 1400, t: "The destination bank applies its own checks before crediting the account." },
    ],
  },
};
...
function SimPayments() {
  ...
  useEffect(() => { setStep(-1); setRunning(false); }, [route]);

  useEffect(() => {
    if (!running) return;
    if (step >= r.steps.length - 1) { setRunning(false); return; }
    const d = r.steps[Math.max(0, step + 1)].ms;
    timer.current = window.setTimeout(() => setStep((x) => x + 1), Math.min(d, 2000));
    return () => clearTimeout(timer.current);
  }, [running, step, route]);

  const elapsed = r.steps.slice(0, step + 1).reduce((a, x) => a + x.ms, 0);
  const totalMs = r.steps.reduce((a, x) => a + x.ms, 0);

  return (
    <div className="sim">
      <div className="art-tabs" role="tablist" aria-label="Payment route">
        {Object.entries(PAY_ROUTES).map(([k, v]) => (
          <button key={k} role="tab" aria-selected={route === k}
            className={"tab" + (route === k ? " on" : "")} onClick={() => setRoute(k)}>{v.name}</button>
        ))}
      </div>

      <div className="pay-head">
        <div><span>Route</span><b>{r.name}</b></div>
        <div><span>Real-world time</span><b className={r.tone}>{r.total}</b></div>
        <div><span>Steps</span><b>{r.steps.length}</b></div>
      </div>

      <div className="pay-track">
        <div className="pay-fill" style={{ width: `${totalMs ? (elapsed / totalMs) * 100 : 0}%` }} />
      </div>
      ...
      <div className="pay-steps">
        {r.steps.map((x, i) => {
          ...
          const cur = i === step;
          return (
            <div className={"pay-step" + (done ? " done" : "") + (cur ? " cur" : "")} key={x.s}>
              <span className="pay-n">{String(i + 1).padStart(2, "0")}</span>
              <div style={{ minWidth: 0 }}>
                <p className="pay-s">{x.s}</p>
                <p className="pay-t">{x.t}</p>
              </div>
      ...
      <div className="art-tabs" style={{ marginTop: 18 }}>
        <button className={"tbtn" + (running ? " on" : "")}
          onClick={() => { if (step >= r.steps.length - 1) setStep(-1); setRunning(!running); }}>
          {running ? "Pause" : "Send the payment"}
        </button>
        <button className="tbtn" onClick={() => { setRunning(false); setStep(-1); }}>Reset</button>
      </div>

      <p className="sim-note">{r.note}</p>
      <p className="sim-note" style={{ marginTop: 10 }}>
        Timings shown are illustrative of the sequence, not measured. What matters is the number of
        separate parties involved: a domestic payment crosses one system, a cross-border payment
        crosses several, and each handover is a place where the payment waits.
      </p>
    </div>
  );
}
...
function SimulatorPage() {
  ...
  const [seg, setSeg] = useState("index");
      ...
      <div className="sim">
        ...
        <div className="sim-grid">
          <aside className="sim-watch">
            <div className="segbar" role="tablist" aria-label="Segment">
              {[["index", "Index"], ["option", "Options"], ["commodity", "Commodities"], ["currency", "Currency"]].map(([k, l]) => (
                <button key={k} role="tab" aria-selected={seg === k}
                  className={"segb" + (seg === k ? " on" : "")}
                  onClick={() => {
                    setSeg(k);
                    const first = INSTRUMENTS.find((x) => x.kind === k);
                    if (first) { setSel(first.id); setLimit(0); }
                  }}>{l}</button>
              ))}
            </div>
            {INSTRUMENTS.filter((i) => i.kind === seg).map((i) => {
              const p = prices[i.id];
...
const PAY_FACTS = [
  { k: "Parties involved", upi: "3", card: "5", cross: "6 or more",
    why: "Every handover between institutions is a place a payment waits." },
  { k: "Systems crossed", upi: "One", card: "Two", cross: "Several",
    why: "A domestic payment stays inside one set of rails governed by one regulator." },
  { k: "Currencies", upi: "One", card: "One, usually", cross: "Two",
    why: "Conversion introduces a rate, a spread and an institution deciding both." },
  { k: "Regulators", upi: "One", card: "One or two", cross: "Two or more",
    why: "Each jurisdiction applies its own compliance checks independently." },
  { k: "Customer sees", upi: "Under 2 seconds", card: "Instant approval", cross: "Pending" },
  { k: "Money actually arrives", upi: "Same day", card: "1 to 3 days", cross: "1 to 5 working days" },
];
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <Reveal delay={120}>
          <p className="lede" style={{ marginTop: 18, maxWidth: "64ch" }}>
            A payment feels like one action. It is four: instruction, authentication, clearing and
            settlement. How long it takes depends almost entirely on how many separate institutions
            have to agree along the way.
          </p>
        </Reveal>
      </div>
      ...
      <div className="wrap" style={{ paddingTop: 40, paddingBottom: 100 }}>
        <Reveal>
          <div className="tabs">
            {[["simulate", "Send a payment"], ["compare", "Why the difference"], ["learn", "The concepts"]].map(([k, l]) => (
        ...
        {tab === "compare" && (
          <Reveal key="c">
            <div className="paycmp">
              <div className="paycmp-head">
                <span />
                <span className="pc-h upi">UPI, domestic</span>
                <span className="pc-h">Card</span>
                <span className="pc-h cross">Cross-border</span>
              </div>
              {PAY_FACTS.map((f) => (
                <div className="paycmp-row" key={f.k}>
                  <span className="pc-k">{f.k}</span>
                  <span className="pc-v upi">{f.upi}</span>
                  <span className="pc-v">{f.card}</span>
                  <span className="pc-v cross">{f.cross}</span>
                  {f.why && <p className="pc-why">{f.why}</p>}
                </div>
              ))}
              <p className="sim-note" style={{ marginTop: 22 }}>
                Nothing about a cross-border payment is technically harder. The delay comes from the
                number of independent parties that must each verify, convert and settle, in different
                working hours, under different rules. Reducing that number is what every reform in
                cross-border payments has attempted.
              </p>
            </div>
          </Reveal>
        )}
        ...
        {tab === "learn" && (
          <Reveal key="l">
            <div className="grid g2" style={{ marginTop: 8 }}>
              {["digital-payments", "payment-rails", "settlement-systems", "real-time-settlement", "digital-money", "data-security"]
                .map((id) => conceptById(id) && <ConceptCard key={id} c={conceptById(id)} />)}
            </div>
          </Reveal>
        )}
      </div>
