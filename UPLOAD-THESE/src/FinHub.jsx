...
/* A trade network that is always alive. Ports light in sequence, value travels
   the routes continuously, and the network densifies as the eras progress.
   Nothing here depends on scrolling, so it never sits frozen. */
function TradeNetwork({ progress = 0, ports = [], label }) {
  const reduced = useReducedMotion();
  const p = Math.max(0, Math.min(1, progress));
  const n = ports.length || 6;
  const reached = Math.round(p * (n - 1));

  const nodes = ports.map((name, i) => {
    const t = i / Math.max(1, n - 1);
    return { name, x: 8 + t * 84, y: 50 + Math.sin(t * Math.PI * 1.6) * 20 - 6, i };
  });
  ...
  nodes.forEach((a, i) => {
    nodes.slice(i + 1).forEach((b) => {
      const span = b.i - a.i;
      if (span <= 2) routes.push({ a, b, span, live: b.i <= reached });
    });
  });

  return (
    <div className="net">
      <svg viewBox="0 0 100 82" className="net-svg" preserveAspectRatio="xMidYMid meet"
        role="img" aria-label="A trade network in which value moves between ports">
        <defs>
          <radialGradient id="net-glow"><stop offset="0%" stopColor="var(--teal)" stopOpacity=".30" />
            <stop offset="100%" stopColor="var(--teal)" stopOpacity="0" /></radialGradient>
          <radialGradient id="net-warm"><stop offset="0%" stopColor="var(--amber)" stopOpacity=".38" />
            <stop offset="100%" stopColor="var(--amber)" stopOpacity="0" /></radialGradient>
        </defs>

        {/* drifting field behind, for depth */}
        {!reduced && (
          <g className="net-field">
            {Array.from({ length: 34 }).map((_, i) => (
              <circle key={i} cx={(i * 37) % 100} cy={(i * 23) % 78} r={0.35 + (i % 3) * 0.22}
                className="net-dust" style={{ animationDelay: `-${(i * 0.7) % 14}s` }} />
            ))}
          </g>
        )}
        ...
        {routes.map((r, i) => {
          ...
          const my = (r.a.y + r.b.y) / 2 - 8 - r.span * 3;
          const d = `M${r.a.x} ${r.a.y} Q${mx} ${my} ${r.b.x} ${r.b.y}`;
          return (
            <g key={i}>
              <path d={d} className={"net-route" + (r.live ? " live" : "")} />
              {r.live && !reduced && (
                <circle r={r.span === 1 ? 1.05 : 0.8} className="net-cargo">
                  <animateMotion dur={`${3.4 + r.span * 1.5}s`} repeatCount="indefinite"
                    begin={`${i * 0.45}s`} path={d} />
                </circle>
              )}
            </g>
          );
        ...
        {nodes.map((nd) => {
          const on = nd.i <= reached;
          const cur = nd.i === reached;
          return (
            <g key={nd.name} transform={`translate(${nd.x} ${nd.y})`}
              className={"net-node" + (on ? " on" : "") + (cur ? " cur" : "")}>
              <circle r="9" fill={cur ? "url(#net-warm)" : "url(#net-glow)"} className="net-halo" />
              <circle r="2.6" className="net-ring" />
              <circle r="1.3" className="net-core" />
              <text y="-5.4" textAnchor="middle" className="net-label">{nd.name}</text>
            </g>
          );
        })}
      </svg>
      {label && <p className="voy-label" style={{ padding: "16px 20px", borderTop: "1px solid var(--line)" }}>{label}</p>}
    </div>
  );
}

/* The market itself changing shape: scattered dealing becomes a floor, a
   screen, then a continuous stream. Always in motion. */
function MarketEvolution({ progress = 0, stages = [], label }) {
  const reduced = useReducedMotion();
  const p = Math.max(0, Math.min(1, progress));
  const idx = Math.min(stages.length - 1, Math.round(p * (stages.length - 1)));

  const pts = Array.from({ length: 48 }, (_, i) => {
    const scattered = { x: 8 + ((i * 31) % 84), y: 14 + ((i * 47) % 54) };
    const floor = { x: 12 + (i % 12) * 6.6, y: 24 + Math.floor(i / 12) * 11 };
    const screen = { x: 10 + (i % 16) * 5.2, y: 22 + Math.floor(i / 16) * 12 };
    const stream = { x: 6 + (i / 47) * 88, y: 42 + Math.sin(i / 3.2) * 15 };
    return [scattered, floor, screen, stream][Math.min(3, idx)];
  });

  return (
    <div className="net">
      <svg viewBox="0 0 100 72" className="net-svg" preserveAspectRatio="xMidYMid meet"
        role="img" aria-label="The market changing shape across eras">
        {idx >= 3 && (
          <path className="mev-stream"
            d={"M" + pts.map((q) => `${q.x.toFixed(1)} ${q.y.toFixed(1)}`).join(" L")} />
        )}
        {pts.map((q, i) => (
          <circle key={i} cx={q.x} cy={q.y} r={idx === 0 ? 1.1 : 0.9}
            className={"mev-dot s" + idx}
            style={{ transitionDelay: `${(i % 12) * 22}ms`,
                     animationDelay: reduced ? undefined : `-${(i * 0.31) % 4}s` }} />
        ))}
        <text x="50" y="66" textAnchor="middle" className="mev-stage">
          {stages[idx] || ""}
        </text>
      </svg>
      {label && <p className="voy-label" style={{ padding: "16px 20px", borderTop: "1px solid var(--line)" }}>{label}</p>}
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <Reveal delay={220}>
          <TradeNetwork progress={progress}
            ports={["Coinage", "Statecraft", "Hundi", "Indian Ocean", "Exchanges", "Central banks", "Today"]}
            label="Value moved along trade routes for four thousand years. The hundi crossed the Indian Ocean while bills of exchange crossed the Mediterranean, and both were solving the same problem. Each port lights as its era arrives, and the network grows denser as you read." />
        </Reveal>
      </div>
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <Reveal delay={170}>
          <MarketEvolution progress={progress}
            stages={["Scattered dealing, no common price", "The trading floor, one place", "Screens, price visible to everyone", "A continuous stream that never sleeps"]}
            label="The same participants, arranged differently in each era. Scattered dealing became a floor, the floor became a matching engine, and the engine became a continuous stream. Watch the shape change as you move through the eras." />
        </Reveal>
      </div>
